...
namespace CNTKUtil
{
    ...
    public abstract class TrainingEngine
    {
        ...
        /// </summary>
        public (float X, float Y)[] TrainingCurve = null;

        /// <summary>
        /// The testing curve with loss and accuracy values.
        /// </summary>
        public (float X, float Y)[] TestingCurve = null;
        ...
        public void Train(double threshold = 0)
        {
            ...
            TrainingCurve = new (float, float)[NumberOfEpochs];
            TestingCurve = new (float, float)[NumberOfEpochs];
            ...
            for (int epoch = 0; epoch < NumberOfEpochs; epoch++)
            {
                ...
                var epoch_test_metric = EvaluateBatches();

                // add to training curve
                TrainingCurve[epoch] = (X: epoch, Y: (float)epoch_training_metric);
                TestingCurve[epoch] = (X: epoch, Y: (float)epoch_test_metric);

                // write current loss and accuracy
                var elapsedTime = DateTime.Now.Subtract(epoch_start_time);
                if (MetricType == MetricTypeEnum.Accuracy)
                {
                    Console.WriteLine($"Epoch {epoch + 1:D2}/{NumberOfEpochs}, Elapsed time: {elapsedTime.TotalSeconds:F3} seconds. " +
                      $"Training accuracy: {epoch_training_metric:F3}. Test accuracy: {epoch_test_metric:F3}.");
                }
                else
                {
                    Console.WriteLine($"Epoch {epoch + 1:D2}/{NumberOfEpochs}, Elapsed time: {elapsedTime.TotalSeconds:F3} seconds. " +
                      $"Training loss: {epoch_training_metric:F3}. Test loss: {epoch_test_metric:F3}.");
                }
        ...
        double TrainBatches()
        {
            ...
            trainingFeatures.Index().Shuffle().Batch(
                BatchSize,
                (Action<int[], int, int>)((indices, begin, end) =>
                {
                    ...
                    // update metric
                    var minibatch_metric = (this.MetricType == TrainingEngine.MetricTypeEnum.Loss) ? trainer.PreviousMinibatchLossAverage() : trainer.PreviousMinibatchEvaluationAverage();
                    metric += minibatch_metric * (end - begin);
